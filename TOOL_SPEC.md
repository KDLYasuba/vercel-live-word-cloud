...
Vercel環境変数 `RESET_PASSWORD` と一致した場合のみ、そのルームの投稿が削除されます。
...
APIはSupabaseから指定ルームの最新500件を取得し、Node.js側でワードごとに集計します。
...
現在の実装では、500件を超える投稿がある場合、最新500件の範囲で集計されます。
